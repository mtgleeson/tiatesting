 ...
 * system-property gate. The listener ships in tia-junit5-perforce's
 * META-INF/services descriptor, so it must be a no-op for any test run that has
 * the jar on the classpath without Tia being explicitly enabled - otherwise IDE
 * runs (or any run that doesn't activate the Tia profile) would trigger Tia
 * bootstrapping.
 */
