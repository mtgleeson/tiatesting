...
Rather than teach every caller (Maven mojos, Gradle tasks, test-runner listeners) how to choose a mode, the choice is resolved once in `H2ConnectionSettings`. It exposes `embedded(dbFilePath)`, `server(url, user, password)` and `fromConfig(dbFilePath, dbUrl, dbUser, dbPassword)` (picks server iff a URL is supplied). It has no system-property entry point of its own: `DataStoreFactory.fromSystemProperties` is the listener's, and the password reaches it as a file path rather than as a value (see [Keeping the password out of checked-in config](../README.md#keeping-the-password-out-of-checked-in-config)). None of these take a branch - `H2ConnectionSettings` only resolves *how to connect* (embedded vs server, URL, credentials); it has no opinion on branch isolation. `H2ConnectionProvider` takes a settings object and builds the JDBC URL from it, and stops caring how the mode was chosen.
...
Server mode needs a username and password, and the obvious place to put them - the `tia { dbPassword = '...' }` block or the POM `<tiaDBPassword>` - is checked into source control.

**This resolution is no longer H2-specific and no longer lives here.** It moved to `CredentialResolver`, called by `DataStoreFactory.fromConfig` *before* it branches on the dialect. While the fallback lived inside `H2ConnectionSettings.server(...)`, only the H2 branch reached it, so a Postgres or generic-JDBC build silently ignored `TIA_DB_USER` / `TIA_DB_PASSWORD` and had no way to keep its password out of checked-in config - it was forced onto `<tiaDBPassword>`, the one channel that used to leak. Every dialect now resolves identically: the configured value, then the environment variable, then a default. The environment lookup is passed in via a package-private overload so the precedence logic is unit-tested without mutating the real process environment.

One credential default stays H2-specific and is deliberately not generalised: the `tia` username. It is H2's convention, and applying it to another vendor would attempt a login as a user that vendor was never told about, so `DataStoreFactory` passes it as the default only on the H2 path and leaves a non-H2 username unset.

For the full set of channels a build can supply the password through - the environment variable, a password file, and Maven's `settings.xml` `<server>` entries - and for how the value reaches the forked test JVM without becoming a system property, see [Keeping the password out of checked-in config](../README.md#keeping-the-password-out-of-checked-in-config). Tia never logs the password - only the JDBC URL - so the one remaining footgun is embedding credentials inside `dbUrl` itself.

The password resolver deliberately distinguishes *not configured* from *configured as empty*, which the username resolver does not. H2 accepts an empty password, so `resolvePassword` treats only `null` as "fall back to the environment"; any non-null configured value - including `""` - is used verbatim and is never trimmed (whitespace can be significant in a password). That lets a build pin an empty password explicitly (`dbPassword = ''` / `<tiaDBPassword></tiaDBPassword>`) and bypass `TIA_DB_PASSWORD`.

The null-vs-empty distinction has to survive the fork boundary, and that constrains how each plugin forwards. Both key their forwarding off *whether a password was configured at all*, not off the resolved value being non-empty: Maven stages and forwards a file even for an explicitly empty password, and Gradle emits the worker environment entry even when the value is `""`. Forwarding nothing in that case would let a `TIA_DB_PASSWORD` that happened to be set in the surrounding environment win inside the fork while the build JVM used the empty value, and the two would connect as different users. An empty *username* is meaningless to H2, so the username keeps the simpler blank-is-unset rule.
...
The column is part of the `tia_core` DDL in `createTiaDB`. Tia is pre-release with no external databases to preserve, so there is no schema migration for it - a database is simply created with the column. A genuinely unset branch (a database seeded before any run sealed a commit) is stored as SQL `NULL` rather than the literal text `null`; the three summary reports render it as `N/A`.
