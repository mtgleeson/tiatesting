...
The summary reports use the same division. `All tests run time` is the serial baseline spread across the groups the most recent mapping-owning all-tests run used (`ReportUtils.lastAllTestsRunGroupCount`), with that count in brackets. When that is more than one group, `All tests run time (1 group)` follows with the serial baseline itself. `Total savings over all runs` sums the wall-clock savings. The baseline is a running average over every all-tests run, not the last one's time; only its group count comes from the last run.
