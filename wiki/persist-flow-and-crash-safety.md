...
- **`persistTestSuites` (tia_test_suite, the row itself)**: one accumulating upsert per suite via `SqlDialect.accumulatingTestSuiteUpsert`, atomic per statement. The run stats *add* to whatever the row holds rather than replacing it, so the values bound are the run's own contribution; the coverage edges and the developer-disabled flag are replaced, since those are what this run observed. `unsealed` is deliberately not part of the statement - it is set only by `persistTestSuiteClasses` and cleared only by `clearUnsealedTestSuites`.
...
- **The suite stats accumulate, and this is the one write whose SQL genuinely differs per vendor.** H2's `MERGE ... KEY(...) VALUES (...)` cannot express it: its right-hand side is a value list, so a column cannot reference its own stored value (`Column "NUM_RUNS" not found`). It uses the SQL-standard `MERGE ... USING ... WHEN MATCHED THEN UPDATE ... WHEN NOT MATCHED THEN INSERT` instead, with the parameters cast explicitly in the `USING` select list because H2 cannot infer a bare parameter's type there. Postgres uses `INSERT ... ON CONFLICT DO UPDATE`, whose assignments may reference both the stored row and `EXCLUDED`. Both forms return the row's generated key for the matched and unmatched case alike, so the edge write still gets the suite id without a second lookup. The run time is a weighted mean guarded for a zero denominator, so a write contributing no run - a retry, or a row written only because its developer-disabled flag changed - leaves the average untouched; stored values are `COALESCE`d from null, because `col + ?` against a null column silently yields null on both vendors rather than failing. `PostgresAccumulatingSuiteStatsTest` re-runs the H2 assertions against Postgres, because two hand-written statements doing the same arithmetic is exactly the shape that drifts.
