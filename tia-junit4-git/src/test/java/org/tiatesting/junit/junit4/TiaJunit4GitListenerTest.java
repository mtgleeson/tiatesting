 ...
 *
 * <p>Surefire instantiates this listener from its {@code listener} property, which it reads
 * regardless of anything Tia configures - unlike the JUnit 5 side, where the launcher session
 * listener only constructs its listener when {@code tiaEnabled} is true. So a disabled build
 * constructs this class anyway, and a constructor needing a repository fails a test run that had
 * asked Tia to stay out of the way entirely.
 */
class TiaJunit4GitListenerTest {
    ...
    void constructingTheListener_withTiaDisabled_opensNoRepository() {
        ...
        // when
        TiaJunit4GitListener listener = new TiaJunit4GitListener();
     ...
     * Those two values are the only thing this listener ever needed the repository for, so with them
     * supplied it must construct in a tree that has none.
     */
    @Test
