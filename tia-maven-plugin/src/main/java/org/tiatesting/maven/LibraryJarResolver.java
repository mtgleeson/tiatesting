 ...
 * JAR file paths by loading the source project's pom.xml via Maven's {@link ProjectBuilder}
...
class LibraryJarResolver {
     ...
     *
     * @return the joined CSV, or {@code null} when {@code tiaSourceLibsCsv} is blank.
     */
    String resolveLibraryJarsCsv(String tiaSourceLibsCsv, String sourceProjectDir){
        ...
        if (!pomFile.isFile()){
            log.warn("Source project pom.xml not found at " + pomFile.getAbsolutePath()
                    + " — skipping tiaSourceLibs resolution.");
            return null;
        }
