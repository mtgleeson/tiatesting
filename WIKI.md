...
- [Setting up a machine to run the release tasks (GPG signing)](#chapter-setting-up-a-machine-to-run-the-release-tasks-gpg-signing)
...
### The select-tests preview and library changes

`select-tests` (the Maven goal / Gradle task) is a read-only preview: it runs with `updateDBMapping=false` and must not write to the mapping DB. That flag governs the whole library flow, and getting library changes to show up in the preview without writing anything takes a dedicated path.

The naive behaviour misses changes. A library-owned diff is partitioned out of the source-project bucket (correctly - a library change is not a source-project change), so it never triggers direct test selection. On a primary build the change is instead *stamped* as a persisted pending batch and drained later; but a preview build persists nothing, so a library change appearing for the first time in the analyzed range has no pending row for the drain to read. The covering tests would silently be absent from `testsToRun` - and therefore from the run-time estimate. That matters when a tool trusts the estimate (for example, sizing build-runner resources from the previewed selection): the estimate would under-report.

The fix is an in-memory drain preview that mirrors the real run without touching the store:

1. **Build, don't persist.** In preview mode the library diffs are still analyzed (`findMethodsImpacted` runs exactly as on a primary build), but instead of stamping a persisted row, Tia builds the batch in memory via `PendingLibraryImpactedMethodsRecorder.buildPendingBatch` - the same pure builder the recorder uses, minus the persist and minus the high-water-mark advance. These are the *synthetic* batches.
2. **Evaluate synthetic + persisted together.** `PendingLibraryImpactedMethodsDrainer.previewTestsForBatches` unions the synthetic batches with the library's already-persisted pending batches (merging by `stampVersion`, mirroring the recorder's MERGE semantics) and runs each through the *same* `shouldDrainBatch` decision the real drain uses. The tests it returns are added to `testsToRun` before the estimate is computed, so the estimate reflects them.

**"What would run now" semantics.** The preview counts only the batches that would actually drain against the *currently resolved* library version - exactly the set the real test run would execute this build. Every hold in the drain rules still applies: a stamp above the resolved version, an `unknownNextVersion` batch whose resolved version has not advanced past the stamp, a resolved version equal to `last_source_project_version`, or an unchanged SNAPSHOT JAR hash all keep their tests *out* of the preview. This is deliberate: the estimate is for this build, so a change that drains only on a future version bump must not inflate it. (The alternative - counting every un-drained pending row, i.e. the full backlog - answers a different question, test-debt forecasting, and is not what the estimate is for.)

**No-mutation guarantee.** The preview path performs no writes at all: no pending rows are stamped, the high-water mark and `last_source_project_version` are never advanced, no drained batch is deleted, and the commit value is never sealed. The primary build (`updateDBMapping=true`) remains the sole writer, and its behaviour is unchanged - it stamps first-seen changes as persisted rows and drains them with a `LibraryImpactDrainResult` for post-run cleanup, exactly as before. Because both paths share `buildPendingBatch` and `shouldDrainBatch`, the preview and the real run agree on which tests a given range selects.

**Known limitation.** The preview can only reason about libraries that a prior primary build has already registered in `tia_library`. Tracked-library reconcile is itself a mapping write, so it is gated on `updateDBMapping` and does not run in the preview. A library that has never been reconciled is invisible to `select-tests` until one primary build registers it; only changes to *already-tracked* libraries are surfaced in the preview.
...
## Chapter: Static test selection — user-declared change-to-suite rules
...
## Chapter: Setting up a machine to run the release tasks (GPG signing)
