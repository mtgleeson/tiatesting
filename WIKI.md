...
The column is added with the same `IF NOT EXISTS` migration style as the other post-1.0 schema additions: `ensureTiaCoreBranchColumnExists` issues `ALTER TABLE tia_core ADD COLUMN IF NOT EXISTS branch` and is called from the load path, from `getTiaCore`, and before each core write, so a database created before this feature gains a NULL-valued column on first contact and is back-filled on the next mapping-update run. A genuinely unset branch (e.g. a stats-only run that never seals a commit) is stored as SQL `NULL` rather than the literal text `null`; the three summary reports render it as `N/A`.
