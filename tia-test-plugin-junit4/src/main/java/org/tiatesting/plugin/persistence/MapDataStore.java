...
public class MapDataStore implements DataStore {
    ...
    @Override
    public StoredMapping getTestMapping() {
    ...
    @Override
    public boolean persistTestMapping(Map<String, Set<String>> testMethodsCalled) {
        ...
        storedMapping.setCommitValue("123456789"); // TODO hook up with the logic to read the workspace commit number from either a passed in parameter or check local workspace
    ...
    private boolean writeTestMappingToDisk(StoredMapping storedMapping){
        ...
        try (FileOutputStream fileOutputStream = new FileOutputStream(dataStorePath + "/" + mappingFilename);
             FileChannel channel = fileOutputStream.getChannel();
             FileLock lock = channel.lock()) {
            ObjectOutputStream out = new ObjectOutputStream(fileOutputStream);
            out.writeObject(storedMapping);
            log.info("Serialized data is saved in " + dataStorePath + "/" + mappingFilename);
        } catch (IOException e) {
            savedToDisk = false;
            log.error("Serialized data failed to saved to disk for " + dataStorePath + "/" + mappingFilename);
            e.printStackTrace();
        }
