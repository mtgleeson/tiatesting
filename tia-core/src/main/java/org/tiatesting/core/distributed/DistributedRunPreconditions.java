...
/**
 * Enforces the three distributed-run validation rules {@link DistributedRunConfig} deliberately
 * does not check, because it is built from raw property values and has no way to know any of
 * them: whether Tia is enabled at all, the resolved datastore, and whether local-changes checking
 * is enabled are all plugin-layer concerns. Task 2 (the Maven goal) and Task 3 (the Gradle task)
 * both call {@link #check} before doing anything else - in particular before the datastore is
 * opened - so a misconfigured or disabled distributed run fails fast with an actionable message
 * instead of silently producing a broken plan, a corrupted diff, or a claimable run persisted to
 * the database while Tia is switched off.
 *
...
public final class DistributedRunPreconditions {
     ...
     * rule 1 (Tia enabled) before rule 2 (shared database) before rule 3 (local-changes checking
     * off), so a configuration that breaks more than one rule fails on the most fundamental one -
     * a disabled Tia makes the database and local-changes questions moot, and a database no runner
     * can share makes the local-changes question moot.
     *
     ...
     *         the property to set; or if the resolved datastore is embedded H2, naming server-mode
     *         H2 and Postgres as the options and {@code tiaDBUrl} as the property that selects
     *         them; or if {@code checkLocalChanges} is true, naming {@code tiaCheckLocalChanges}
     ...
     */
    public static void check(final boolean tiaEnabled, final String dbUrl, final String dialectOverride,
